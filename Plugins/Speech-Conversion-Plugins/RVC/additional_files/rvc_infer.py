                ...
                if (
                    ...
                    for config_file in ["32k.json", "40k.json", "48k.json"]:
                        ...
                        with open(f"configs/{config_file}", "w") as f:
                            f.write(strr)
                    # not sure why this is needed. file got moved to /infer/modules/train/preprocess.py
                    # https://github.com/RVC-Project/Retrieval-based-Voice-Conversion-WebUI/commit/2c4ec6db930f91832f4164009903a5f272979c1e
...
rmvpe_model_path = os.path.join(rvc_sts_plugin_dir, "Retrieval-based-Voice-Conversion-WebUI", "assets", "rmvpe")
os.environ["rmvpe_root"] = rmvpe_model_path

# realtime mode imports
import tools.rvc_for_realtime_patched as rvc_for_realtime
...
def release_model():
    global hubert_model
    if hasattr(hubert_model, 'model'):
        del hubert_model.model
    if hasattr(hubert_model, 'feature_extractor'):
        del hubert_model.feature_extractor
    if hasattr(hubert_model, 'hf_tokenizer'):
        del hubert_model.hf_tokenizer
    #del hubert_model
    hubert_model = None
    if torch.cuda.is_available():
