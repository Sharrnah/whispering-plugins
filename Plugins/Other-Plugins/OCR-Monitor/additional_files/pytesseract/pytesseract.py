...
def set_tesseract_cmd(cmd):
    """
    Set the tesseract command to be used by pytesseract.
    """
    global tesseract_cmd
    if not isinstance(cmd, str):
        raise TypeError('Tesseract command must be a string')
    tesseract_cmd = cmd

def set_tessdata_dir(dir):
    global tessdata_dir
    if not isinstance(dir, str):
        raise TypeError('tessdata_dir must be a string')
    tessdata_dir = dir
    #os.environ["TESSDATA_PREFIX"] = str(dir)
...
def run_tesseract(
    ...
    cmd_args += (tesseract_cmd,)
    if tessdata_dir != '':
        cmd_args += ('--tessdata-dir', tessdata_dir)
    cmd_args += (input_filename, output_filename_base)
